...
export const template = (params : Params) => {
  ...
  paramsStore[id_g] = {
    id : id_g,
    path : params.path ? [...params.path, id_g] : ['default', 'input', id_g],
...
export const template_flat = (params : Params) => {
  ...
  paramsStore[id_g] = {
    id : id_g,
    path : params.path ? [...params.path, id_g] : ['default', 'input', 'id'],
    style :  params.style || {},
    placeholder : params.placeholder || '',
    type: params.type || 'text',
    data: params.data || {}
  }
  return `<div class="${className()}" id="${id_g}"> INPUT_TEMPLATE </div>`
}
