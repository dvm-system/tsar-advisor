const __component = function(id){
  ...
  const placeholder = __component.state.params[id].placeholder
  const current = __component.state.data[id].value
  const state = __component.state.data[id].state
  const style = Object.entries(__component.state.params[id].style).reduce((acc,cur)=>{
    return acc + cur[0] + ': ' + cur[1] + ';'
  }, '')
  const type = __component.state.params[id].type
  return `
    <input
      type="${type}"
      class="form-control"
